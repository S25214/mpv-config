...
function toggle_crop(mode)
    ...
    local remove_filter = function()
        mp.osd_message("Reset Crop/Zoom")
        local pan_x = mp.get_property_number("video-pan-x", 0)
        local pan_y = mp.get_property_number("video-pan-y", 0)
        local zoom_v = mp.get_property_number("video-zoom", 0)
        local vf_table = mp.get_property_native("vf")
        if #vf_table > 0 or pan_x ~= 0 or pan_y ~= 0 or zoom_v ~= 0 then
    ...
    if not remove_filter() then
        start_crop(mode)
        if mode == "hard" then
            mp.osd_message("Start Crop")
        else
            mp.osd_message("Start Zoom")
        end
    end
end
